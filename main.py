...
hg['movie'].y = labels                       # Labels for 'movie' nodes
...
def test() -> Dict[str, List[float]]:
    ...
    for split in ['train_mask', 'val_mask', 'test_mask']:
        ...
        if split == 'test_mask':
            ...
            if incorrect.any():
                idx = torch.where(incorrect)[0][torch.randint(0, incorrect.sum(), (1,))].item()  # Get single index
...
for epoch in range(1, 200):
    ...
    print(f'Micro F1 -> Train: {metrics["micro_f1"][0]:.4f}, Val: {metrics["micro_f1"][1]:.4f}, Test: {metrics["micro_f1"][2]:.4f}')
    print(f'Macro F1 -> Train: {metrics["macro_f1"][0]:.4f}, Val: {metrics["macro_f1"][1]:.4f}, Test: {metrics["macro_f1"][2]:.4f}')

    # Store best model based on macro F1 (better for imbalanced classes)
